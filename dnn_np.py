...
class NeuralNet(object):
    ...
    def compute_loss(self, y, y_hat):
        ...
        eps = 1e-7
        data_loss = -np.mean(np.sum(y*np.log(y_hat+eps), axis=1))
...
def minibatch_train(net, train_x, train_y, cfg):
    ...
    for e in range(cfg.num_epoch):
        for i in range(0, train_x.shape[0], cfg.batch_size):
            x_batch = train_x[i:i+cfg.batch_size]
            y_batch = train_y[i:i+cfg.batch_size]
            y_batch = create_one_hot(y_batch, net.num_class)
            all_x = net.forward(x_batch)
            y_hat = all_x[-1]
            loss = net.compute_loss(y_batch, y_hat)
            grads = net.backward(y_batch, all_x)
        ...
        # In kết quả dự đoán và nhãn thực
        y_hat_train = np.argmax(net.forward(train_x)[-1], axis=1)
        print(f"Epoch {e+1}: loss is {loss:.5f}")
